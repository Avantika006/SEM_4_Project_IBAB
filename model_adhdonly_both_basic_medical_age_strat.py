...
# df = df[(df['age_at_eval_years'] >= 0) & (df['age_at_eval_years'] <= 4)]
# df = df[(df['age_at_eval_years'] > 4) & (df['age_at_eval_years'] <= 7)]
df = df[(df['age_at_eval_years'] > 7) & (df['age_at_eval_years'] <= 10)]
...
#     fig1.savefig("waterfall_plot_model1.png", bbox_inches='tight')
#
...
#     fig4 = plt.figure()
#     shap.dependence_plot("gest_age", exp.values, X_test_original, interaction_index="med_cond_birth", show=False)
...
# #Implementing class-balance -2 reverse for the age group 0-2 years
# # Separate the dataset into ADHD-affected and unaffected individuals
# adhd_only = df[df['prediction'] == 0]
# asd_adhd = df[df['prediction'] == 1]
#
# # Randomly sample an equal number of rows from the adhd only individuals subset
# adhd_only_sampled = adhd_only.sample(n=both, random_state=42)
#
# # Concatenate the ASD-affected subset and the randomly sampled unaffected individuals subset
# merged_df_1 = pd.concat([asd_adhd, adhd_only_sampled])
#
# # Shuffle the rows of the final dataset
# merged_df = merged_df_1.sample(frac=1, random_state=42).reset_index(drop=True)
